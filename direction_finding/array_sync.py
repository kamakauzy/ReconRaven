...
class SDRArraySync:
    ...
    def _load_calibration(self):
        ...
        try:
            ...
            if cal and cal['num_sdrs'] == self.num_elements:
                ...
                logger.info(f"Loaded DF calibration: {cal['calibration_method']} from {cal['created_at']}")
    ...
    def calibrate_phase(self, frequency_hz: float, num_samples: int = 10000,
                       known_bearing: float = None, save_to_db: bool = True) -> bool:
        ...
        try:
            ...
            self.is_calibrated = True
            logger.info(f"Phase calibration complete!")
            logger.info(f"  Phase offsets: {self.phase_offsets}")
            ...
            if save_to_db:
                try:
                    ...
                    cal_method = f"Cross-correlation"
                    if known_bearing is not None:
