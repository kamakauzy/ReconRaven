...
class CorrelationEngine:
    ...
    def find_temporal_correlations(self, time_window_seconds: int = 10) -> List[Dict]:
        ...
        for (freq1, freq2), data in correlations.items():
            ...
            results.append({
                ...
                'confidence': min(data['count'] / 10.0, 1.0),  # More occurrences = higher confidence
    ...
    def get_device_behavior_profile(self, frequency_hz: float) -> Dict:
        ...
        return {
            ...
            'transmission_rate_per_hour': len(detections) / max((timestamps[-1] - timestamps[0]).total_seconds() / 3600, 1),
    ...
    def _classify_device_behavior(self, avg_interval: float, is_periodic: bool, 
                                  burst_count: int, total_detections: int) -> str:
    ...
    def build_device_network(self) -> Dict:
        ...
        for freq in nodes:
            ...
            node_data.append({
                ...
                'name': device.get('name', f'{freq/1e6:.3f} MHz') if device else f'{freq/1e6:.3f} MHz',
        ...
        # Mark top 20% as hubs
        if connection_count:
            hub_threshold = sorted(connection_count.values(), reverse=True)[int(len(connection_count) * 0.2)] if len(connection_count) > 5 else 2
    ...
    if args.correlations:
        ...
        if 'error' in profile:
            ...
            if profile['is_periodic']:
                ...
                print(f"  Interval: {profile['avg_interval_sec']:.1f}s (±{profile['std_interval_sec']:.1f}s)")
