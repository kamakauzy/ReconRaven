...
class SpectrumScanner(DebugHelper):
    ...
    def _detect_peaks(self, psd: np.ndarray, center_freq: float) -> list[SignalHit]:
        ...
        try:
            ...
            freqs = np.linspace(center_freq - freq_span / 2, center_freq + freq_span / 2, len(psd))
    ...
    def _estimate_bandwidth(self, psd: np.ndarray, peak_idx: int, peak_power: float) -> float:
    ...
    def _scan_band_list_multi(self, bands: list[dict[str, Any]]) -> dict[str, list[SignalHit]]:
        ...
        def scan_worker(sdr_idx, assigned_bands):
            ...
            for band in assigned_bands:
                ...
                original_freq = (
                    self.sdr.sdrs[sdr_idx].center_freq if sdr_idx < len(self.sdr.sdrs) else None
                )
