...
class VoiceMonitor:
        ...
        try:
            ...
            if auto_record:
                ...
                output_file = os.path.join(
                    self.recording_dir,
                    f"voice_{freq_mhz:.3f}MHz_{timestamp}.wav"
                )
            ...
            device_name = device.get('name', f'{freq_mhz:.3f} MHz') if device else f'{freq_mhz:.3f} MHz'
    ...
    def scan_voice_bands(self, band: str = '2m', dwell_time_sec: int = 5):
        ...
        voice_freqs = {
            '2m': [  # 2-meter ham band (FM voice)
                ...
                146.400, 146.420, 146.440, 146.460, 146.480, 146.500,  # Simplex
            ...
            'gmrs': [  # GMRS channels
                462.550, 462.575, 462.600, 462.625, 462.650,
                462.675, 462.700, 462.725,
                467.550, 467.575, 467.600, 467.625, 467.650,
                467.675, 467.700, 467.725
            ],
            ...
            'marine': [  # Marine VHF
                156.300, 156.400, 156.450, 156.500,  # Common channels
                156.550, 156.600, 156.650, 156.700,
                156.750, 156.800, 156.850, 156.900
            ]
        }
    ...
    def monitor_multiple_frequencies(self, frequencies_mhz: List[float], 
                                    mode: str = 'FM', auto_record: bool = True):
        ...
        """
        print(f"\n{'='*70}")
        print(f"MULTI-FREQUENCY VOICE MONITOR")
        print(f"{'='*70}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dwell', type=int, default=5, 
                       help='Dwell time per frequency when scanning (seconds)')
